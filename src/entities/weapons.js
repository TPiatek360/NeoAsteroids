...
export class EnemyBullet {
    constructor(ship, angle = ship.a) {
        this.ownerId = 'enemy';
        this.x = ship.x + ship.r * Math.cos(angle);
...
export class Rocket {
    ...
    draw() {
        ...
        ctx.fillStyle = NEON_ENEMY_ROCKET;
        ctx.shadowColor = NEON_ENEMY_ROCKET;
        ctx.shadowBlur = 10;
    ...
    update(deltaTime) {
        ...
        if (this.acquisitionTimer > 0) {
            ...
            if (!this.target) {
                ...
                if (this.target.constructor.name === 'Asteroid' && !gameState.asteroids.includes(this.target)) {
                    targetIsInvalid = true;
                } else if (gameState.enemies.includes(this.target) && this.target.isDead) {
                    targetIsInvalid = true;
                } else if (this.target.constructor.name === 'Player' && (this.target.lives <= 0 || !gameState.players.includes(this.target))) {
                    targetIsInvalid = true;
                }
            }
...
export class EnemyMissile {
    ...
    draw() {
        ...
        ctx.translate(this.x, this.y);
        ctx.rotate(this.a);
        ctx.fillStyle = NEON_ENEMY_MISSILE;
        ctx.shadowColor = NEON_ENEMY_MISSILE;
        ctx.shadowBlur = 10;
        ctx.beginPath();
        ctx.moveTo(this.r * 1.5, 0);
        ctx.lineTo(-this.r * 0.75, this.r * 0.75);
...
export class Bomb {
    ...
    draw() {
        ...
        ctx.fillStyle = `rgba(255, 170, 0, ${0.3 * alpha})`;
        ctx.lineWidth = 2;
        ctx.shadowColor = this.color;
        ctx.shadowBlur = 15;
        ctx.beginPath();
