...
export const gameState = {
    ...
    // --- Input & UI State ---
    keys: {},
    activeGamepads: {}, // This cache is now the source of truth for assignments
...
export function resetGameState() {
    ...
    if (gameState.gameMode === 'singlePlayer') {
        ...
        if (gamepadIndices.length > 0) {
            ...
            console.log(`Assigning Keyboard 1 + Gamepad ${gamepadIndices[0]} to Player 1`);
        } else {
        ...
        if (gamepadIndices.length >= 2) {
            ...
            console.log(`Gamepad ${gamepadIndices[0]} -> P1, Gamepad ${gamepadIndices[1]} -> P2`);
        } else if (gamepadIndices.length === 1) {
            p1_inputConfig = { type: 'gamepad', index: gamepadIndices[0] };
            p2_inputConfig = { type: 'keyboard2' };
            console.log(`Gamepad ${gamepadIndices[0]} -> P1, Keyboard 2 -> P2`);
        } else {
            p1_inputConfig = { type: 'keyboard1' };
            p2_inputConfig = { type: 'keyboard2' };
            console.log('Keyboard 1 -> P1, Keyboard 2 -> P2');
        }
